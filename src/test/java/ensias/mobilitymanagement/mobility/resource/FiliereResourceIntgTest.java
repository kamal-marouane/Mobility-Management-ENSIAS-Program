...
public class FiliereResourceIntgTest {
    ...
    /*
    @Test
    @DisplayName("POST /filiere/add - Success")
    void testAddFiliere() throws Exception {
        Filiere newFiliere = Filiere.builder().filiereName("Physique").build();
        when(filiereService.addFiliere(any(Filiere.class))).thenReturn(newFiliere);

        mockMvc.perform(post("/filiere/add")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(asJsonString(newFiliere)))
                .andExpect(status().isCreated())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.filiereId").value(0L))
                .andExpect(jsonPath("$.filiereName").value("Physique"));
    }
