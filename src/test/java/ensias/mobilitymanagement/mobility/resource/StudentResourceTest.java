...
class StudentResourceTest {
    ...
    @Test
    void getFiliereById() {
        String filiereName = "filiereName";
        List<Student> expectedStudents = Arrays.asList(new Student(), new Student());
        when(studentService.getStudentRepository().findStudentByFiliereFiliereName(filiereName)).thenReturn(expectedStudents);

        ResponseEntity<List<Student>> response = studentResource.getFiliereById(filiereName);

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(expectedStudents, response.getBody());
        verify(studentService.getStudentRepository(), times(1)).findStudentByFiliereFiliereName(filiereName);
    }
    */
...
/*
    ...
    void deleteStudent() {
        ...
        student.setFiliere(new Filiere("Test Filiere"));

        Student createdStudent = studentService.addStudent(student);

        studentService.deleteStudent(createdStudent.getStudentId());

        Optional<Student> deletedStudent = studentService.getStudentRepository().findById(createdStudent.getStudentId());
